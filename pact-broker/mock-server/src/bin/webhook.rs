...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/api/flux-notification/clear", post(clear_flux_notifications))
        ...
        .route("/api/argo-notification/clear", post(clear_argo_notifications))
        ...
        .route("/api/{notification_type}/clear", post(clear_generic_notifications))
        .with_state(state);
...
fn validate_fluxcd_payload(payload: &serde_json::Value) -> Result<(), String> {
    ...
    if let Some(severity) = payload.get("severity").and_then(|v| v.as_str()) {
        if !["info", "warning", "error"].contains(&severity.to_lowercase().as_str()) {
            return Err(format!("Invalid severity value: {}. Must be one of: info, warning, error", severity));
        }
    }
...
async fn clear_generic_notifications(
    ...
    info!("🗑️  Cleared {} notifications for type: {}", count, notification_type);
    ResponseJson(serde_json::json!({
        "cleared": count,
...
mod tests {
    ...
    async fn test_flux_webhook_invalid_severity() {
        ...
        payload.as_object_mut().unwrap().insert("severity".to_string(), json!("invalid"));
    ...
    async fn test_validate_fluxcd_payload() {
        ...
        invalid3.as_object_mut().unwrap().insert("severity".to_string(), json!("invalid"));
        assert!(validate_fluxcd_payload(&invalid3).is_err());

        // Valid severity values
        for severity in ["info", "warning", "error"] {
            let mut payload = create_fluxcd_payload();
            payload.as_object_mut().unwrap().insert("severity".to_string(), json!(severity));
